...
#define ADDR_INIT_FLAG  161    // 아직 쓰지 않은 구역(160번까지 사용)
#define INIT_MAGIC      0xA5   // '설정 완료'를 의미하는 값
...
void setup() {
  ...
  if (EEPROM.read(ADDR_INIT_FLAG) != INIT_MAGIC) {
    ...
    apSSID      = ""; // AP SSID는 랜덤 생성 함수에서 결정

    // AP SSID 랜덤 숫자로 자동 생성
    //generateAPSSID();
  ...
  if (mode == 0) {
    ...
    for (int i = 0; i < 5; i++) {
      ledcWrite(LED_PIN, 0);    // LED 끄기
      delay(200);
      ledcWrite(LED_PIN, 255);  // LED 켜기
      delay(200);
    }
    ledcWrite(LED_PIN, 0);
...
void setAPSSIDwithMAC() {
  ...
  // ESP32의 STA(Station) MAC 주소를 읽어옵니다.
  // (AP MAC을 쓰려면 'WIFI_IF_AP' 사용)
